...
import { postAddCoTanDetail } from "../../../Utility/Services/CoApplicant";
...
const Tan = props => {
	...
	const CoHandleSubmit = async () => {
		setState({ ...state, loading: true });
		if (form.tanApplicable == false) {
			let obj1 = {
				leadCode: match.params.leadcode,
				mobileNumber: match.params.comobileno,
			...
			let res = await postAddCoTanDetail(obj1).then(res => res);
			if (res.error) {
			...
			let res = await postAddCoTanDetail(obj).then(res => res);
			if (res.error) {
				setState({ ...state, loading: false });
				return;
			}
