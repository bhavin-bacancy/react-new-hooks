...
import { postAddCoPanDetail } from "../../Utility/Services/CoApplicant";
...
class PanGST extends React.Component {
	constructor() {
		...
		this.state = {
			...
			updateLoading: false,
			bttransferflag: false
		};
	}
	SaveCoDetail = () => {
		let { entityForm, panForm, gstForm, entity } = this.state;
		let { match, selectedEntity } = this.props;
		...
		postAddCoPanDetail({
			financialstatus: this.state.checkData,
			//typeOfEntity: this.state.checkData,
			...
			if (res.data.error) {
				...
				ts(res.data.message);
				this.setState(
					{ saveDetailLoading: false, saveDetailStatus: true },
					() => {
	...
	SaveDetail = () => {
		let { entityForm, panForm, gstForm, entity, bttransferflag } = this.state;
		...
		postAddPanDetail({
			...
			defaultbranch: localStorage.getItem("employeeId"),
			bttransferflag: this.state.bttransferflag,
			mainapplicant: true,
			custcode: ""
		}).then(res => {
			if (res.error) {
	...
	onInputValidate = (name, error, formName) => {
		// if (name === "panNumber") {
		//   var pan_value = this.state.panForm.panNumber.toUpperCase();
		...
		//   let company = "";
		//   let pass = false;
		//   if (this.state.entityForm.typeOfEntity == "4") {
		//     company = "C";
		//   } else if (this.state.entityForm.typeOfEntity == "5") {
		//     company = "P";
		//   } else if (this.state.entityForm.typeOfEntity == "6") {
		//     company = "H";
		//   } else if (this.state.entityForm.typeOfEntity == "7") {
		//     company = "F";
		//   } else if (this.state.entityForm.typeOfEntity == "8") {
		//     company = "F";
		//   } else if (this.state.entityForm.typeOfEntity == "1") {
		//     company = "P";
		//   } else if (this.state.entityForm.typeOfEntity == "10") {
		//     company = pan_value[3] == "P" ? "56" : pan_value[3];
		//   } else {
		//     pass = true;
		//   }
		//   if ((pan_value.match(reg) && pan_value[3] == company) || pass) {
		//     error = "";
		//   } else {
	...
	componentDidMount() {
		...
		// if(preProps.commonData.addressline1!=this.props.commonData.addressline1)
		// {
	...
	FormUpdate = name => {
		...
		if (leadDetail.bttransferflag === true) {
			this.setState({
				bttransferflag: true
			});
		} else {
	...
	render() {
		...
		let CheckForm = Object.keys(panForm).filter(res => {
			if (res != "errors" && !panForm[res]) {
				if (entityForm.typeOfEntity != 5) {
					return res != "firmName"
				}
				return res;
			}
							...
							<div className="row align-items-center">
												...
												{saveEntity ? (
													<label className="main styleGreen text-primary pl-4 my-lg-4 my-3">
														<input
															...
															name="bt-flag"
															value="bt-flag"
															checked={this.state.bttransferflag}
															onChange={this.bankFlagChange}
															disabled
														/>
														<span className="geekmark"></span>
														<span className="checkboxText">
															Please select if Balance Transfer applicable
                          </span>
													</label>
												) : (
														<>
								...
								{history.location.pathname.startsWith(
									public_url.co_applicant_update_profile
										...
										entityForm.typeOfEntity === "1" && this.state.checkData ? (
											this.state.checkData === "financial" ? (
													...
													<>
														...
														<label className="main styleGreen text-primary pl-4 my-lg-4 my-3">
															<input
																...
																checked={this.state.checkData === "financial"}
															/>
															<span className="geekmark"></span>
															<span className="checkboxText">Financial</span>
														</label>
														<label className="main styleGreen text-primary pl-4 my-lg-4 my-3">
															<input
																type="checkbox"
																...
																value="non-financial"
																onChange={this.handleCheckChange}
																checked={this.state.checkData === "non-financial"}
															/>
															<span className="geekmark"></span>
															<span className="checkboxText">Non-Financial</span>
														</label>
													</>
												) : this.state.checkData ? (
													this.state.checkData
								...
								{history.location.pathname.startsWith(
									public_url.co_applicant_update_profile
										...
										<React.Fragment>
											{this.state.checkData && (
												<div className="col-4 text-right pr-0 mt-3 mt-lg-0">
													<button
														onClick={() => {
															this.props.GetLeadDetail();
														}}
														className="btn btn-secondary btn-rounded ls-1 cursor-pointer fs-16 btn-green bg-green"
													>
														Cancel
                        </button>
												</div>
											...
											<div
												className={`${
													this.state.checkData ? "col-2 " : "col-12"
													}  text-right pr-0 mt-3 mt-lg-0`}
											>
												{leadDetail &&
													leadDetail.pansectionflag &&
													!this.state.checkData && (
														<button
															className={`btn btn-secondary btn-rounded ls-1 cursor-pointer fs-16 btn-green bg-success mr-3`}
															onClick={() => {
																this.props.GetLeadDetail();
															}}
														>
												...
												<button
													disabled={!this.state.checkData}
													className={`btn btn-secondary btn-rounded ls-1 cursor-pointer fs-16 ${this
														.state.checkData && "btn-green"}`}
													onClick={this.ResetEnityInindividual}
												>
													Save
                      </button>
											</div>
										...
										history.location.pathname.startsWith(
											...
											<div className={`col-12 text-right mt-4 mt-lg-0`}>
												<button
...
export default withRouter(PanGST);
