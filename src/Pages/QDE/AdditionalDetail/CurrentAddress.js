...
class CurrentAddress extends React.Component {
	...
	SaveCoApplicantForm = () => {
		...
		if (!saveForm) {
			...
			postCoApplicantCurrentAddress(obj).then(res => {
				if (res.error) {
	...
	FormUpdate = () => {
		...
		if (this.props.commonData && this.props.commonData.addressline1 && !leadDetail.curraddflag) {
			saveFormFlag = false;
		} else {
