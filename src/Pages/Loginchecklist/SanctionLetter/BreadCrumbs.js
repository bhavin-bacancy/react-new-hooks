import React from "react";
import { Link } from "react-router-dom";
import { public_url } from "../../../Utility/Constant";

export const BreadCrumbs = (props) => {
	let { match, sanctionDetail, status, history } = props;
	return (
		<>
			<section className="bg_l-secondary pt-4">
				<div className="container  ">
					<div className="d-flex justify-content-start align-items-center">
						<div className="breadcrums d-flex align-items-center flex-wrap">
							<ul>
								<li className="mr-1" class="active">
									<Link to={`${public_url.prospect_list}`}>
										LAP Prospects
                      </Link>
								</li>
								<li className="mr-1" class="active">
									<Link to={`${public_url.co_applicant_status}/${match.params.leadcode}`}>
										Profile
                      </Link>
								</li>
								<li className="mr-1" class="active">
									<Link to={`${public_url.sanction_letter}/${match.params.leadcode}`}>
										Sanction Letter
									</Link>
								</li>
							</ul>
							{sanctionDetail && sanctionDetail.applicant && sanctionDetail.applicant.loannumber && (
								<span className="note ml-3 mt-md-0 mt-3">
									Reference ID : {sanctionDetail.applicant.loannumber}
								</span>
							)}
						</div>
					</div>
				</div>
			</section>
		</>
	);
};
