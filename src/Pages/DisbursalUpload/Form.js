...
const FormComponent = props => {
	...
	const FormDataUpdate = res => {
		let selectedDocument = options.filter(res => {
			if (res.docTypeDesc.includes(props.subsectionName) && res.selected)
				return res;
		});
	...
	const AddDocument = (state, setState, props, utilState, utilSetState) => {
		...
		state.file.map((res, index) => {
			if (res && typeof res == "object") {
				zip.file(`${index + res.name}`, res, { binary: true });
			} else {
				...
				uploadString: {
					...
					docData: {
						docSubCategoryId: options.filter(res => {
							if (res.docTypeDesc.includes(props.subsectionName)) return res;
						})[0].docSubCategoryId,
	...
	return (
		<>
			...
			{props.sectionSwitch && <hr class="bg_lightblue border-0 h-1px" />}
		</>
	);
};
